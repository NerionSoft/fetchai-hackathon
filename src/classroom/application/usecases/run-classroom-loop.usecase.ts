...
import type { PdfCodex } from "../ports/pdf-codex";
...
/**
 * One piece of raw source material entering the loop. Text is used verbatim; a
 * PDF is first turned into markdown (see the ingest step). Several sources are
 * merged into a single lesson that runs the loop ONCE.
 */
...
export interface RunClassroomLoopParams {
  /** Lesson id (stable across a run); generated by the delivery adapter. */
  id: string;
  /** Optional explicit title; when omitted it is inferred from the ingested markdown. */
  title?: string;
  /** The raw source material — one or more text/PDF inputs. */
...
export class RunClassroomLoopUseCase {
  constructor(
    ...
    private readonly pdfCodex: PdfCodex,
    private readonly concurrency: number = DEFAULT_CONCURRENCY,
  ) {}

  async execute({ id, title, sources, sink }: RunClassroomLoopParams): Promise<LoopResult> {
    const lesson = await this.ingest({ id, title, sources });
  ...
  /**
   * Turn the raw sources into a single lesson. PDF sources are converted to
   * markdown through the PdfCodex port; text sources are used verbatim. All
   * sources are concatenated (in order) into one document — the loop runs once
   * over the merged lesson. Title is the caller's, else inferred from the markdown.
   */
  private async ingest({ id, title, sources }: Pick<RunClassroomLoopParams, "id" | "title" | "sources">): Promise<Lesson> {
    const parts = await mapPool(sources, this.concurrency, async (source) =>
      source.kind === "pdf" ? this.pdfCodex.pdfToMarkdown(source.bytes) : source.text,
    );
    const markdown = parts.map((p) => p.trim()).filter(Boolean).join("\n\n");
    if (!markdown) throw new Error("No lesson content could be read from the provided inputs.");
    return { id, title: title?.trim() || titleFromMarkdown(markdown), markdown };
  }
