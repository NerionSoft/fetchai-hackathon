/**
 * ClassroomSim — SSE wire protocol (shared by the server use case and the
 * client). One JSON object per SSE message. The scene (SVG circles) and result
 * panels are driven entirely by this event stream. It is the output contract of
 * the RunEventSink port. Pure types + pure (de)serialization only.
 */
import type {
  ClassId,
  FactCheckReport,
