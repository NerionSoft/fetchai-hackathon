 ...
 * corresponding Zod schema in the classroom domain.
 *
...
function produceStudent(brief: Extract<AgentBrief, { role: "student" }>): StudentRestitution {
...
function produceDiagnosis(
  brief: Extract<AgentBrief, { role: "diagnosis" }>,
): TeacherDiagnosis {
...
function produceRewrite(
  brief: Extract<AgentBrief, { role: "rewrite" }>,
): LessonVersion {
...
function produceFactCheck(
  brief: Extract<AgentBrief, { role: "factcheck" }>,
): FactCheckReport {
...
function produceEvaluations(
  brief: Extract<AgentBrief, { role: "evaluations" }>,
): EvaluationSet {
  const { lessonMarkdown, lessonTitle, diagnosis } = brief;
  const lesson = analyzeLesson(lessonMarkdown, lessonTitle);
  const flaws = detectFlaws(lesson);
...
function produceExercises(
  brief: Extract<AgentBrief, { role: "exercises" }>,
): ExerciseSet {
  const { lessonMarkdown, lessonTitle, diagnosis } = brief;
  const lesson = analyzeLesson(lessonMarkdown, lessonTitle);
  const flaws = detectFlaws(lesson);
...
function produceSheet(
  brief: Extract<AgentBrief, { role: "sheet" }>,
): RevisionSheet {
  const { lessonMarkdown, lessonTitle, diagnosis } = brief;
  const lesson = analyzeLesson(lessonMarkdown, lessonTitle);
  const flaws = detectFlaws(lesson);
...
/**
 * Turn a decoded AgentBrief into schema-valid output. Deterministic and pure.
 * The return type is intentionally `unknown` (matching the requested API): the
 * caller validates it against the role's Zod schema.
 */
export function produceMock(brief: AgentBrief): unknown {
  switch (brief.role) {
    case "student":
